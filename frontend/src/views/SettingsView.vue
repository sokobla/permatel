  ...
  <div class="set-page">
    ...
    >
      <v-tab value="general" class="text-none"
        ><v-icon start size="16">mdi-tune</v-icon>Général</v-tab
      >
      <v-tab value="smtp" class="text-none"
        ><v-icon start size="16">mdi-email-arrow-right-outline</v-icon
        >SMTP</v-tab
      >
      <v-tab value="emails" class="text-none"
        ><v-icon start size="16">mdi-email-edit-outline</v-icon>Emails</v-tab
      >
      <v-tab v-if="sections.imap" value="imap" class="text-none"
        ><v-icon start size="16">mdi-email-arrow-left-outline</v-icon
        >IMAP</v-tab
      >
      <v-tab value="reference" class="text-none"
        ><v-icon start size="16">mdi-format-list-bulleted</v-icon>Valeurs de
        référence</v-tab
      >
      <v-tab value="sla" class="text-none"
        ><v-icon start size="16">mdi-timer-alert-outline</v-icon>SLA</v-tab
      >
      <v-tab v-if="sections.integrations" value="integrations" class="text-none"
        ><v-icon start size="16">mdi-puzzle-outline</v-icon>Intégrations</v-tab
      >
    </v-tabs>
...
const tab = ref(
  availableTabs.value.includes(requested) ? requested : "general",
);
