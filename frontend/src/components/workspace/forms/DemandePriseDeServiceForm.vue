  ...
  <div class="df-root pds-root">
    ...
    <div class="pds-body">
      ...
      <div class="pds-grid">
        <div class="form-group">
          <label class="form-label"
            >CLIENT <span class="pds-required">*</span></label
          >
          <ClientCombobox
        ...
        <div class="form-group">
          <label class="form-label"
            >SITE <span class="pds-required">*</span></label
          >
          <SiteSelect
            :client-id="clientId"
      ...
      <p class="pds-hint">
        ...
        qu'une). « Terminer » clôture la vacation en cours de cet agent.
      </p>
    </div>
...
const canStart = computed(
  () => !!(props.agentId && clientId.value && siteId.value),
);
...
async function checkOpenVacation() {
  ...
  try {
    const open = await listPrisesDeService({
...
.pds-btn-cancel:hover:not(:disabled) {
  background: rgba(0, 0, 0, 0.04);
}
.pds-btn-end {
  background: #fff;
  border-color: rgba(0, 11, 35, 0.18);
  color: #000b23;
}
.pds-btn-end:hover:not(:disabled) {
  background: #000b23;
  color: #fff;
}
.pds-btn-start {
...
.pds-btn-start:hover:not(:disabled) {
  background: #009090;
}
