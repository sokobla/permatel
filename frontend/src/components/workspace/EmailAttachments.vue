<template>
  <div class="ea-root">
    <div class="ea-bar">
      ...
      <span v-if="modelValue.length" class="ea-count"
        >{{ modelValue.length }} fichier(s)</span
      >
      <input
        ref="picker"
        type="file"
...
const ACCEPT =
  ".pdf,.png,.jpg,.jpeg,.webp,.gif,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.txt,.csv,.zip";
...
function onPick(e) {
  ...
  for (const f of incoming) {
    if (!ALLOWED.has(ext(f.name))) {
      error.value = `Type non autorisé : ${f.name}`;
      continue;
    }
    if (f.size > props.maxSizeMb * 1024 * 1024) {
      error.value = `Trop volumineux (max ${props.maxSizeMb} Mo) : ${f.name}`;
      continue;
    }
