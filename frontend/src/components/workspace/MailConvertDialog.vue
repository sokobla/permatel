  ...
  >
    <v-card rounded="lg" class="mcv-card">
      <v-card-title class="mcv-title">
        Traiter l'email
        <button class="mcv-close" @click="emit('update:modelValue', false)">
      ...
      <v-card-text class="mcv-body">
        ...
        <p class="mcv-source">
          ...
          <span class="mcv-from">{{ email?.from_address }}</span>
        </p>

        <!-- Choix du mode -->
        <div class="mcv-modes">
          <button
            :class="['mcv-mode', { 'mcv-mode--on': mode === 'existing' }]"
            @click="mode = 'existing'"
          >
            <v-icon size="16">mdi-link-variant</v-icon> Demande existante
          </button>
          <button
            :class="['mcv-mode', { 'mcv-mode--on': mode === 'new' }]"
            @click="mode = 'new'"
          >
        ...
        <div v-else class="mcv-section">
          <v-row dense>
            <v-col cols="12" sm="6">
              <label class="mcv-label"
                >Type <span class="mcv-req">*</span></label
              >
              <v-select
            ...
            <v-col cols="12" sm="6">
              <label class="mcv-label"
                >Client <span class="mcv-req">*</span></label
              >
              <ClientCombobox
                @client-selected="onClient"
                @client-cleared="form.client_id = null"
              />
            </v-col>
            <v-col cols="12">
              <label class="mcv-label"
                >Intitulé <span class="mcv-req">*</span></label
              >
              <v-text-field
                v-model="form.titre"
                variant="outlined"
...
async function confirm() {
  ...
  try {
    ...
    if (mode.value === "existing") {
      if (!targetDemande.value) {
        error.value = "Sélectionnez une demande.";
        return;
      }
      ...
      if (!form.titre.trim()) {
        error.value = "L'intitulé est requis.";
        return;
      }
