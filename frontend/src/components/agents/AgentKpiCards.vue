  ...
  <div class="akpi">
    ...
    <template v-else-if="kpis">
      <div class="akpi-grid">
        ...
        <div class="akpi-card akpi-card--score">
          <div class="akpi-card__top">
            ...
            <v-tooltip
              text="100 − 5 points par incident discriminant sur la période. Borné 0–100."
            >
              <template #activator="{ props }"
                ><v-icon v-bind="props" size="13" color="#9aa0aa"
                  >mdi-information-outline</v-icon
                ></template
              >
            </v-tooltip>
          </div>
          <div class="akpi-card__value" :style="{ color: scoreColor }">
            {{ kpis.score }}<small>/100</small>
          </div>
        </div>
        ...
        <div class="akpi-card akpi-card--incident">
          <div class="akpi-card__top">
            ...
            <v-tooltip
              text="Anomalies discriminantes (natures pénalisantes ou impact sécurité). Impactent le score."
            >
              <template #activator="{ props }"
                ><v-icon v-bind="props" size="13" color="#9aa0aa"
                  >mdi-information-outline</v-icon
          ...
          <div class="akpi-card__value akpi-card__value--red">
            {{ kpis.incidents }}
          </div>
        </div>
        ...
        <div class="akpi-card">
          <div class="akpi-card__top">
            ...
            <v-tooltip
              text="Toutes les anomalies impliquant l'agent (incidents inclus). N'impactent pas directement le score."
            >
              <template #activator="{ props }"
                ><v-icon v-bind="props" size="13" color="#9aa0aa"
                  >mdi-information-outline</v-icon
      ...
      <p class="akpi-relation">
        {{ kpis.incidents }} incident(s) discriminant(s) sur
        {{ kpis.anomalies }} anomalie(s).
      </p>
    </template>
...
.akpi-card__value small {
  font-size: 14px;
  font-weight: 600;
  color: #9aa0aa;
}
.akpi-card__value--red {
  color: #e74c3c;
}
.akpi-relation {
  font-size: 11.5px;
