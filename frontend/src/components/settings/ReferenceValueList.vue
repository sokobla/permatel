      ...
      <tbody>
        <tr v-for="item in items" :key="item.id">
          <td class="rvl-td-label">
            <span :class="{ 'rvl-inactive': !item.active }">{{
              item.label
            }}</span>
          </td>
          <td class="rvl-td-status">
            <v-chip
              size="x-small"
              :color="item.active ? '#22c55e' : '#9aa0aa'"
              variant="tonal"
            >
          ...
          <td class="rvl-td-actions">
            <v-btn
              ...
              size="x-small"
              :title="
                item.is_discriminant
                  ? 'Discriminante — impacte Incidents agent & score'
                  : 'Non discriminante — cliquer pour marquer discriminante'
              "
      ...
      <v-card rounded="lg">
        ...
        <v-card-text>
          <v-form ref="formRef" @submit.prevent="confirm">
            <label class="rvl-label"
              >Libellé <span class="rvl-req">*</span></label
            >
            <v-text-field
              v-model="draft"
              variant="outlined"
    ...
    <v-dialog v-model="deleteOpen" max-width="400">
      <v-card rounded="lg">
        ...
        <v-card-text class="rvl-dlg-text">
          ...
          <strong>« {{ target?.label }} »</strong> ? Cette action est
          définitive.
        </v-card-text>
        <v-divider />
        <v-card-actions>
