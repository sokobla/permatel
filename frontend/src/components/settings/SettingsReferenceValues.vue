  ...
  <v-card variant="flat" class="srv-card" rounded="lg" border>
    <div class="srv-head">
      <div>
        ...
        <p class="srv-sub">
          Gérez les listes de valeurs utilisées dans les formulaires métier.
        </p>
      </div>
    </div>
        ...
        <v-window-item v-for="f in FAMILIES" :key="f.key" :value="f.key">
          <ReferenceValueList
            ...
            @discriminant="
              (item) =>
                onUpdate(f.key, item, {
                  is_discriminant: !item.is_discriminant,
                })
            "
...
const FAMILIES = [
  ...
  {
    key: "qualification_agent",
    label: "Qualifications Agent",
...
const state = reactive(
  Object.fromEntries(
    FAMILIES.map((f) => [
      f.key,
      { items: [], loading: false, saving: false, loaded: false },
    ]),
  ),
);
...
async function onUpdate(key, item, patch) {
  ...
  s.saving = true;
  try {
    const updated = await settingsService.updateReferenceValue(
      key,
      item.id,
      patch,
    );
