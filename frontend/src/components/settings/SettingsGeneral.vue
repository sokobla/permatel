    ...
    <div class="sg-body">
      ...
      <v-form v-else ref="formRef">
        ...
        <label class="sg-label"
          >Nom du tenant <span class="sg-req">*</span></label
        >
        <v-text-field
          v-model="form.nom"
          variant="outlined"
        ...
        <v-text-field
          ...
          variant="outlined"
          density="comfortable"
          placeholder="support@organisation.com"
          prepend-inner-icon="mdi-lifebuoy"
          :rules="[rules.emailOptional]"
          hide-details="auto"
        />
        <p class="sg-hint">
          Destinataire des demandes « Contacter le support ».
        </p>
      </v-form>
    </div>
...
function onLogo(e) {
  ...
  if (!["image/png", "image/jpeg", "image/webp"].includes(f.type)) {
    logoError.value = "Format non supporté.";
    return;
  }
...
async function load() {
  ...
  try {
    ...
    feedback.type = "error";
    feedback.text = "Impossible de charger les paramètres.";
  } finally {
    loading.value = false;
  }
}
...
async function save() {
  ...
  try {
    ...
    feedback.text = "Paramètres enregistrés.";
  } catch (err) {
    feedback.type = "error";
