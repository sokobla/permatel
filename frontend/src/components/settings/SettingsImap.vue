  ...
  <v-card variant="flat" class="imap-card" rounded="lg" border>
    <div class="imap-head">
      <div>
        ...
        <p class="imap-sub">
          Boîte de service relevée pour collecter les emails entrants.
        </p>
      </div>
      <v-chip
        size="small"
        :color="form.inbound_enabled ? '#22c55e' : '#9aa0aa'"
        variant="tonal"
      >
      ...
      <v-form v-else ref="formRef">
        ...
        <div class="imap-switch">
          <v-switch
            ...
            hide-details
            inset
            label="Activer la collecte des emails entrants"
          />
        </div>

        <v-row dense>
          <v-col cols="12" sm="8">
            <label class="imap-label"
              >Hôte IMAP
              <span v-if="form.inbound_enabled" class="imap-req">*</span></label
            >
            <v-text-field
        ...
        <p class="imap-note">
          La collecte s'exécute via la tâche planifiée
          <code>flask mail-fetch</code> (à venir). Cette configuration prépare
          la réception centralisée par tenant.
        </p>
      </v-form>
    </div>
...
const hostRules = computed(() =>
  form.inbound_enabled ? [rules.required] : [],
);
