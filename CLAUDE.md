...
> `AUDIT_PERMATEL.md`, `ODOO_INTEGRATION_PLAN.md`, `TELEPHONIE_INTEGRATION_PLAN.md`, and `docs/cdc/` are **planning/audit documents, not implementation status** — treat their changelog/phase tables as the most reliable status source (they're actively maintained per-phase), but verify against actual code for anything not covered here. Updated 2026-08-14 (Odoo/ERP Phase 6 foundations now live; Téléphonie extended beyond Phase 14, see below):
> - **Odoo/ERP**: Phase 6 ("Fondations transverses") is built — code deliberately named `Erp*`/`erp_*`, not `Odoo*` (the plan avoids vendor-specific naming, `ODOO_INTEGRATION_PLAN.md` §2.3/§3.B). `backend/app/models/erp.py` (`ErpConfig`, `ErpSyncQueue`), `backend/app/services/erp_client.py` (XML-RPC wrapper, `company_id`-scoped `execute_kw`, injected not singleton), `backend/app/routes/settings.py` (`/settings/erp`, `/settings/general`), `backend/app/routes/erp.py` (audited direct-access for global ADMIN), `flask erp-sync-dispatch`/`flask erp-backfill` CLI commands, `Tenant.channel_erp` flag. **What's still a stub**: `erp-sync-dispatch` has working lock/retry bookkeeping but no real flux logic yet (every queued row currently fails with an explicit "not implemented" message) and `erp-backfill` only counts eligible records — both depend on `erp_partners`/`erp_employees` (mapping tables), which are Phase 7 scope, not yet built. The `x_permatel_ref` custom field (idempotency key) is an **Odoo-side** prerequisite (Odoo Studio/module), not PERMATEL code — undone, can't be verified from this repo. See `ODOO_INTEGRATION_PLAN.md` §5 Phase 6 for the full breakdown. Phases 7–10 (partners/CRM, quotes/billing, HR/timesheets, planning) remain genuinely not started.
