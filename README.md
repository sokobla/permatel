...
**Statut** : Backend ✅ / Frontend ✅ | **Tests** : 394 (backend) + 76 (connecteur) ✅

### Changelog v1.10.0 (13-14 août 2026)
- ☎️ **Module Téléphonie — exécution à distance ESL (login/logout/statut agent, codes de pause)** : PERMATEL peut désormais déclencher des actions côté FusionPBX depuis l'application — jobs `agent_login`/`agent_logout`/`agent_status_change` dispatchés au connecteur via Redis (`_dispatch_pbx_job`), sans table de jobs persistée (la connexion ESL du connecteur est déjà permanente et synchrone, contrairement à un système externe joint en HTTP). Le statut "En pause" porte un `pause_code` (1 caractère) configurable par tenant (`Paramètres > Téléphonie`, table `pbx_pause_codes`, ligne `"0"` protégée non éditable) — un véritable événement CUSTOM FreeSWITCH (`esl_adapter::agent_pause_code`) est injecté par le connecteur puis réingéré par son propre pipeline normal. Au login PERMATEL, statut auto "On Break"/`pause_code="0"`. Nouveau bouton de statut self-service dans l'app-bar (Disponible / En pause avec choix du code / Déconnecté).
- 🐛 **Correctif du round-trip PBX pour le statut agent** : la version initiale ne comptait que sur la confirmation FreeSWITCH → mod_callcenter → ré-ingestion pour que le statut apparaisse quelque part — si le connecteur/Redis/le PBX était injoignable, le statut demandé n'était jamais persisté ni diffusé (bouton "amnésique" après rechargement, supervision temps réel figée). `_record_and_broadcast_agent_status_event()` persiste et diffuse désormais l'intention de l'utilisateur immédiatement, indépendamment du round-trip PBX (qui reste la source de vérité effective, sans effet une fois rejoué).
- 🔗 **Statut PBX synchronisé avec le cycle de vie des sessions PERMATEL** : `UserSession.status` (le champ `PAUSED`, présent depuis longtemps — sweep d'inactivité, `/refresh`, badges UI — mais jamais alimenté) bascule désormais réellement en `PAUSED`/`ACTIVE` en parallèle des changements de statut PBX (self-service, login/logout auto). Nouveau helper `_session_status_durations()` reconstruisant les minutes actif/pause/déconnecté d'une session depuis l'historique `TelephonyEvent` (`user_session_id`, colonne existante jamais alimentée jusqu'ici). `/api/auth/sessions/stats` expose `total_active_min`/`total_pause_min` ; `/api/auth/sessions/monitoring` (+ export CSV) expose `active_minutes`/`pause_minutes` par session — nouvelles cartes KPI et colonne dans les Rapports/Supervision correspondants.
- 🐛 **Correctif du filtrage agent/file de l'historique des appels (CDR)** : `agent_login`/`queue_id` ne sont peuplés que sur une partie des événements d'un appel (l'événement terminal issu du connecteur ESL ne les porte jamais) — un filtre SQL au niveau ligne faisait donc disparaître l'appel entier au lieu de l'inclure/exclure correctement après regroupement. Filtres déplacés après la fusion des événements, comme `call_status`/`direction`/`search` déjà en place.
- 📊 **Rapport "Prises de service" enrichi** : nouveau tableau détaillé téléchargeable en CSV (`Rapports > Prises de service`) — date, agent, type d'agent, client, site, début/fin, et **utilisateur PERMATEL ayant déclaré chaque étape** (nom/prénom). Nouvelle colonne `prises_de_service.ended_by_id`, symétrique à `created_by_id` déjà existant.
- 🌍 **Géolocalisation IP (drapeau pays) dans les Rapports** : lookup MaxMind GeoLite2-Country **100% local** (`maxminddb`, aucune IP transmise à un tiers — choix délibéré pour une app de sécurité), dégradation gracieuse si la base `.mmdb` est absente (fichier non versionné, licence MaxMind — à obtenir via un compte gratuit et déposer dans `backend/geoip/`, monté en lecture seule dans le conteneur `backend`). `country_code` exposé en JSON, volontairement absent de tous les exports CSV.
- 📞 **`RingingCallBar`** : nouveau bandeau global (toutes les pages, comme `CallCardBar`) affiché uniquement pour l'agent dont l'appel sonne encore (avant décroché) — accent ambre, distinct de `CallCardBar` (appel déjà en cours, qui reste l'indicateur pour le reste du cycle de l'appel).
- 🎛️ **Refonte du filtre CDR** (`Rapports > Téléphonie`) : champs de date Vuetify (au lieu d'inputs natifs mal alignés avec le reste de la barre), bouton "Filtrer" explicite (le filtrage ne se déclenchait auparavant que sur la pagination), Agent/File transformés en multi-select alimentés par deux nouveaux endpoints légers (`/telephony/agents/roster`, `/telephony/queues/roster`) réutilisant les helpers d'alias déjà en place — aucune nouvelle requête inventée.
- 🗃️ Migrations : `af1a8735c279` (`telephony_events.pause_code` + `pbx_pause_codes`), `b7c9e1a3f5d7` (`prises_de_service.ended_by_id`).
- 142 nouveaux tests backend (394 au total), 18 nouveaux tests connecteur (76 au total).

### Changelog v1.9.0 (29-30 juillet 2026)
- ☎️ **Module Téléphonie — corrélation de files d'attente, validée contre du trafic FusionPBX réel** : (1) `bridge-agent-start` (jusqu'ici abandonné comme CC-Action non reconnue) est l'unique source du chemin d'enregistrement d'un appel de file — `variable_record_file_path` ne se peuple **jamais**, y compris sur un appel dont l'enregistrement existe bien sur disque ; le vrai chemin est extrait de `variable_execute_on_pre_bridge`, rattaché au call_uuid du membre via `variable_cc_member_session_uuid`. (2) **Appels fantômes éliminés** : chaque tentative de sonnerie d'agent pour un appel de file (`agent-offering` suivi d'un `CHANNEL_CREATE` outbound vers l'extension de l'agent, jamais bridgé en cas d'échec) apparaissait comme une ligne distincte dans `/active-calls` — un appel de file réel montrait 3 lignes au lieu de 2. Le connecteur pose désormais une corrélation courte (extension → session membre) à `agent-offering`, consommée par le `CHANNEL_CREATE` suivant pour le tagger `linked_call_uuid` et le fondre via la règle de fusion existante.
- 🆔 **`User.agent_login` bascule vers l'UUID FusionPBX de l'agent (CC-Agent), plus une extension** : l'annuaire agents du connecteur (`_fetch_agent_list`) filtre désormais les agents découverts par UUID, pas par extension (celle-ci — `station_extension`, déjà existante — n'est que le poste physique associé, peut changer sans que ce soit un changement d'agent). Réel gain : les événements `CALLCENTER_AGENT_STATE_CHANGE`/`CALLCENTER_MEMBER_ENRICHMENT`/`CALLCENTER_BRIDGE_RECORDING` transmettent désormais l'UUID comme `agent_login`, garanti de correspondre à un `User` PERMATEL réel déclaré pour ce tenant (jamais un agent PBX non déclaré).
- 🎨 **Alias agent/file lisibles en Supervision et CDR** : `agent_login` (uuid, illisible tel quel) est désormais habillé du nom PERMATEL de l'agent + son poste déclaré (`_agent_alias_lookup`, même principe que l'alias de file déjà existant) sur `/active-calls`, `/agents/status` et l'historique des appels — le CDR exporté (CSV) n'expose plus que le nom et le poste, jamais l'uuid brut. `/active-calls` expose aussi `queue_label` ("Alias (id)").
- 🐛 **Correctif d'un décalage de +1h sur toutes les durées d'appel affichées en Supervision temps réel** : les timestamps du module Téléphonie (routes + événement WebSocket) sont naïfs-UTC sans désignateur de fuseau — un navigateur les interprète comme heure locale, pas UTC. Suffixe `Z` désormais explicite partout.
- ✨ **Refonte du tableau "Appels en cours"** (colonnes Agent/Appelant/Destination/Queue/Statut/Durée) et **animations d'entrée/sortie/changement de statut** sur les listes temps réel de Supervision (arrivée/fin d'appel, réordonnancement, cross-fade des badges de statut — `prefers-reduced-motion` respecté).
- 📞 **Bandeau d'appel actif** (`CallCardBar`) : se déclenche sur le premier événement WebSocket dont `agent_login` correspond à l'utilisateur connecté, ancré sous la barre d'appli (jamais en superposition), visible sur toutes les pages, action rapide "Créer une demande". Animations sans mesure JS (CSS `grid-template-rows` 0fr↔1fr).
- 🗂️ **Sites/Agents groupés par client/prestataire, Contacts restylé** : reprend le langage visuel de `AnomaliesView`/`OrdersView` (pastille carrée colorée, badge de comptage, lien "Voir la fiche X →") — pixel pour pixel, pas juste "inspiré de". Sites et Agents ont un parent 1–N naturel (Client, Prestataire — bucket "Agent interne (Tenant)" pour les agents sans prestataire) ; Contacts reste à plat (liaison N–N avec les clients) mais reçoit le même langage visuel par ligne. Plus de pagination serveur sur ces tableaux : liste complète filtrée chargée une fois, le bouton "Grouper" ne change que l'affichage.
- 🐛 **`frontend/src/views/crud-view.css` n'était importé par AUCUNE vue** (toutes importent `frontend/src/assets/styles/crud-view.css`) — la refonte "pastille carrée uniforme" du 29/07 (commit `8a5b2cd`) avait donc édité un fichier mort : l'avatar carré 8px, les badges pilule sans bordure et la ligne d'enrichissement sous le nom (`.user-cell__meta`) n'ont **jamais été visibles en production**. Porté dans le fichier réellement importé, fichier orphelin supprimé. Au passage, `table-layout: fixed` restauré (perdu lors du passage de `v-data-table-server` à une `<table>` HTML brute pour le regroupement — les largeurs de colonnes fixées via `style="width: X"` n'étaient plus respectées).
- 13 nouveaux tests backend (252 au total), 14 nouveaux tests connecteur (58 au total).

### Changelog v1.8.0 (28-29 juillet 2026)
- ☎️ **Module Téléphonie — Phase 13 (relocalisation config + Supervision temps réel)** : la configuration Téléphonie quitte l'onglet Paramètres autonome pour un panneau "Configurer" inline sous `Paramètres > Intégrations` (bouton actif dès que le canal téléphonie est activé sur le tenant, indépendamment d'un connecteur déjà configuré). Nouvel onglet `Supervision > Téléphonie` (visible ssi canal actif ET connecteur configuré) : KPIs du jour, appels en cours (temps réel via WebSocket), files d'attente, grille d'état des agents. Le connecteur capte désormais le header FreeSWITCH `CC-Agent-Status` (`mod_callcenter`, jusque-là ignoré) ; `GET /telephony/agents/status` normalise ce statut brut en présence disponible/pause/hors-ligne (défaut hors-ligne si absent/inconnu — jamais fabriqué). Charge par agent = appels traités relatifs au plus sollicité, pas un taux d'occupation (aucune durée continue disponible/occupée suivie).
- ☎️ **Module Téléphonie — Phase 14 (webhook CDR + Rapports)** : nouveau canal d'ingestion complémentaire à l'ESL live, alimenté directement par FusionPBX (`mod_json_cdr`) à la fin de chaque appel — `POST /api/telephony/cdr/ingest/<token>`, jeton **par connecteur** (généré à la création, régénérable, stocké chiffré+redisplayable via "Copier le token" dans le panneau de configuration), restriction IP optionnelle (`authorized_ip`). Un CDR produit jusqu'à 3 événements rétrodatés (ringing/answered/terminal), compatibles sans modification avec les routes KPI existantes. Nouvel onglet `Rapports > Téléphonie` (visible ssi canal actif) : historique des appels paginé/filtrable avec export CSV, section Enregistrements avec écoute/téléchargement (proxy backend, uniquement si le PBX expose une URL http(s) — sinon signalé indisponible plutôt qu'un lien cassé) et export groupé en ZIP (`_indisponibles.txt` listant ce qui a été exclu).
- 🐛 **Six correctifs successifs sur le webhook CDR, chacun validé contre du trafic FusionPBX réel** (voir `TELEPHONIE_INTEGRATION_PLAN.md` §8.6 pour le détail complet) : (1) `request.get_json()` refusait de parser sans `force=True` ; (2) le corps est en réalité `application/x-www-form-urlencoded` (`cdr=<json>`, comportement par défaut de libcurl) et non du JSON brut ; (3) le JSON (~15-80 Ko, dump complet des variables de canal) contient des `&`/`=` littéraux qui faisaient tronquer la valeur par le parseur de formulaire de Werkzeug — remplacé par une extraction directe sur le corps brut (recherche de `{`...`}`, immunisée aux `&`/`=` internes) ; (4) préservation du `+` des numéros E.164 (`unquote` avant `unquote_plus`, pour ne pas convertir un `+` non échappé en espace) ; (5) tolérance des caractères de contrôle bruts non échappés (`json.loads(..., strict=False)`) ; (6) **root cause finale** — FusionPBX interpole certains en-têtes SIP bruts (`sip_full_from`/`sip_full_to`, ex. `"Nom Affiché" <sip:...>;tag=...`) dans le JSON sans échapper les guillemets internes du nom d'affichage, produisant un JSON syntaxiquement invalide à la source ; `_repair_unescaped_quotes()` répare heuristiquement (repère la vraie fin de chaîne, échappe les guillemets internes) en dernier recours.
- 🔍 **Trace diagnostique du webhook CDR** (`TELEPHONY_CDR_TRACE=true`, désactivée par défaut) : journalise l'inventaire complet des variables reçues + écrit le payload intégral dans `/tmp/cdr_trace_last.json`.
- 🐛 **Deux correctifs supplémentaires confirmés grâce à deux traces CDR réelles complètes** (appel direct + appel en file d'attente, voir `TELEPHONIE_INTEGRATION_PLAN.md` §8.7) : `caller_id_number`/`destination_number` n'existent pas systématiquement sous `variables` — extraits désormais depuis `callflow[0].caller_profile`, plus fiable ; `cc_agent` s'est révélé être un UUID interne FusionPBX (pas un login/une extension) — `agent_login` est désormais extrait depuis `callflow[0].caller_profile.originatee` (le leg réellement bridgé par `mod_callcenter`), gardé derrière la présence de `cc_queue` (confirmé fiable, inchangé) pour ne jamais fabriquer de donnée agent hors contexte de file d'attente réel. `recording_url` reste non confirmé (aucun appel de test avec enregistrement actif à ce jour).
- Migrations `9c8b7a6f5e4d` (`authorized_ip`/`cdr_webhook_token(_hash)` sur `pbx_connectors`). 39 nouveaux tests (239 au total).
