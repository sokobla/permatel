...
**Statut** : Backend ✅ / Frontend ✅ | **Tests** : 236 ✅

### Changelog v1.8.0 (28 juillet 2026)
...
- 🔍 **Trace diagnostique du webhook CDR** (`TELEPHONY_CDR_TRACE=true`, désactivée par défaut) : journalise l'inventaire complet des variables reçues + écrit le payload intégral dans `/tmp/cdr_trace_last.json`, pour confirmer quelles variables un PBX réel envoie effectivement avant d'exploiter de nouveaux champs (`cc_queue`/`cc_agent`, exposition des enregistrements).
- Migrations `9c8b7a6f5e4d` (`authorized_ip`/`cdr_webhook_token(_hash)` sur `pbx_connectors`). 37 nouveaux tests (236 au total).
